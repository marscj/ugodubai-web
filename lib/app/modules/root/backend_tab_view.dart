...
class BackendTabView extends StatelessWidget {
  const BackendTabView({Key? key}) : super(key: key);
...
class _DesktopScreenState extends State<DesktopScreen> {
  ...
  void initState() {
    ...
    pages.first.binding?.dependencies();
    ...
    if (page != null && route != null) {
      if (!tabs.any((element) =>
        ...
        setState(() {
          ...
          // Get.put(() => (page as GetPageMenu).controller, tag: page.name);
  ...
  void removePage(GetPage page) {
    ...
    if (_index > 0) {
      // var key = (pages[_index] as GetPageMenu).controllerKey;
      tabs.removeAt(_index);
      pages.removeAt(_index);

      GetInstance().delete(tag: 'OrderListController');
